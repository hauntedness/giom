...
func (p *page) drawIdentitiesItems(gtx layout.Context) layout.Dimensions {
	if len(p.accountsView) == 0 {
		return p.NoAccount.Layout(gtx)
	}
	return p.List.Layout(gtx, len(p.accountsView), func(gtx layout.Context, index int) (d layout.Dimensions) {
...
func (p *page) drawNormalMenuItems(gtx layout.Context) layout.Dimensions {
	...
	if p.btnDeleteAll.Clicked() {
		p.selectAll()
		p.Modal().Show(p.drawDeleteAccountsModal, nil, component.VisibilityAnimation{
			Duration: time.Millisecond * 250,
			State:    component.Invisible,
			Started:  time.Time{},
		})
...
func (p *page) drawDeleteAccountsModal(gtx layout.Context) layout.Dimensions {
	gtx.Constraints.Max.X = int(float32(gtx.Constraints.Max.X) * 0.85)
	gtx.Constraints.Max.Y = int(float32(gtx.Constraints.Max.Y) * 0.85)
	if p.btnYes.Clicked() {
...
func (p *page) handleAddAccountClick(gtx layout.Context) {

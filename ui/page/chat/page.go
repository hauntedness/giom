...
import (
	...
	"github.com/hauntedness/giom/ui/page/chatroom"
...
type page struct {
	...
	*material.Theme
	...
	PasswordForm            api.ViewWithDBListener
...
func (p *page) DrawAppBar(gtx layout.Context) layout.Dimensions {
	if p.btnNavIcon.Clicked() {
		p.Manager.NavigateToUrl(api.SettingsPageURL, nil)
	}
...
func (p *page) drawChatItems(gtx layout.Context) layout.Dimensions {
	...
	return p.List.Layout(gtx, len(p.chatPageItems), func(gtx layout.Context, index int) (d layout.Dimensions) {
...
func (p *page) drawAccountDetailsModal(gtx layout.Context) layout.Dimensions {
	gtx.Constraints.Max.X = int(float32(gtx.Constraints.Max.X) * 0.85)
	gtx.Constraints.Max.Y = int(float32(gtx.Constraints.Max.Y) * 0.85)

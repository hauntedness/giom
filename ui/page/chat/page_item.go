...
func NewChatPageItem(manager api.Manager, contact service.Contact) *pageItem {
...
func (pi *pageItem) Layout(gtx layout.Context) layout.Dimensions {
	pi.fetchLastMessage()
...
func (pi *pageItem) layoutContent(gtx layout.Context) layout.Dimensions {
	...
	if pi.Clickable.Clicked() {
		...
		if pi.CurrentPage().URL() != chatRoomPage.URL() {
			pi.NavigateToUrl(api.ChatPageURL, func() {
			...
			d := flex.Layout(gtx,
				...
				layout.Rigid(func(gtx layout.Context) layout.Dimensions {
					...
					return flex.Layout(gtx,
						...
						layout.Rigid(func(gtx layout.Context) layout.Dimensions {
							chatroomURL := api.ChatRoomPageURL + "/" + api.URL(pi.contact.PublicKey)
...
func (pi *pageItem) URL() api.URL {
	return api.ChatPageURL + "/" + api.URL(pi.contact.PublicKey)
}

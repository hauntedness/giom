...
func draw(gtx layout.Context, app *App) {
	layout.Flex{}.Layout(gtx, layout.Flexed(1, func(gtx layout.Context) layout.Dimensions {
		navBar := component.NewNav("title", "subtitle")
		item := component.NavItem{
			Tag:  app.item1,
			Name: "1st item",
			Icon: common.MustIcon(icons.AVAVTimer),
		}
		navBar.AddNavItem(item)
		navBar.SetNavDestination(app.item1)
		return navBar.Layout(gtx, app.theme, app.anim)
	}))
}
